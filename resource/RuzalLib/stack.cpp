...
int StackDump_(const Stack* stk, const char* file, size_t line, const char* func) {
	...
	printf("%llu\n", *(Canary_t*)((char*)stk->data + stk->capacity * sizeof(Elem_t)));
	printf("}\n");
...
static int PrintErrorInfo(unsigned error, Stack* stk) {
	...
	FILE* f = (stk->logger).file;
	if (check(STK_NULLPTR))  fprintf(f, "\nstack ptr is null\n");
	if (check(DATA_NULLPTR)) fprintf(f, "\nstack->data ptr is null\n");
	if (check(RANGE_ERROR))  fprintf(f, "\nrange error\n");
	if (check(CANARY_ERROR)) fprintf(f, "\ncanary error\n");
	if (check(HASH_ERROR))   fprintf(f, "\nhash mismatch\n");
	#undef check
	return 0;
};
